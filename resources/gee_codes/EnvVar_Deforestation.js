...
var clip_func = function(i){
  var j = i.clip(aoi)
  return j.select(bandList).divide(10000).copyProperties(i)
}

// Applying clipping function
var clipped_ic = yearwise.map(clip_func);
print(clipped_ic)
Map.addLayer(clipped_ic.first(), {min: -0.2, max: 0.8, palette: ['green', 'orange', 'red']}, 'NDVI First')
